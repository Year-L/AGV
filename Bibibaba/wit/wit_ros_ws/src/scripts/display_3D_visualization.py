...
# Main scene
scene=display(title="Wit IMU Main Screen")
...
scene2 = display(title='Wit IMU Roll, Pitch, Yaw',x=550, y=0, width=500, height=500,center=(0,0,0), background=(0,0,0))
scene2.range=(1,1,1)
scene2.select()
#Roll, Pitch, Yaw
...
label(pos=(-0.5,0.6,0),text="Roll (degrees)",box=0,opacity=0)
label(pos=(0.5,0.6,0),text="Pitch (degrees)",box=0,opacity=0)
label(pos=(0.0,0.02,0),text="Yaw (degrees)",box=0,opacity=0)
...
label(pos=(0,-0.8,0),text="Angular Velocity x / y / z (deg/s)",box=0,opacity=0)
...
def processIMU_message(imuMsg):
    ...
    linAccLabel.text ="%8s \t %8s \t %8s" %(str(round(imuMsg.linear_acceleration.x, precision)), str(round(imuMsg.linear_acceleration.y, precision)), str(round(imuMsg.linear_acceleration.z, precision)))
    angVelLabel.text = "%12s \t %12s \t %12s" %(str(round(imuMsg.angular_velocity.x*rad2degrees, precision)) , str(round(imuMsg.angular_velocity.y*rad2degrees, precision)) , str(round(imuMsg.angular_velocity.z*rad2degrees, precision)))
